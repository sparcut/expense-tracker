...
<template>
  <div class="flex flex-wrap items-center gap-2 p-4 bg-card border border-border rounded-lg">
    <input v-model="search" type="text" placeholder="Search title or description..." :class="inputClass + ' flex-1 min-w-48'" />
    ...
    <button @click="toggleSort('date')" :class="['flex items-center gap-1 px-3 py-1.5 rounded-md border text-sm transition-colors', sortField === 'date' ? 'border-primary text-primary bg-primary/10' : 'border-border text-muted-foreground hover:text-foreground']">
      <ArrowUpDown :size="13" /> Date {{ sortField === 'date' ? (sortDir === 'desc' ? '↓' : '↑') : '' }}
    </button>
    <button @click="toggleSort('amount')" :class="['flex items-center gap-1 px-3 py-1.5 rounded-md border text-sm transition-colors', sortField === 'amount' ? 'border-primary text-primary bg-primary/10' : 'border-border text-muted-foreground hover:text-foreground']">
